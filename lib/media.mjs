...
export async function probeMedia(ffprobe, file) {
  ...
  await new Promise((resolve,reject)=>{
    const child=spawn(ffprobe,['-v','error','-select_streams','v:0','-show_entries','stream=width,height:format=duration','-of','json',file],{windowsHide:true});
  ...
  const json=JSON.parse(stdout), stream=json.streams?.[0];
  const result={width:Number(stream?.width),height:Number(stream?.height),duration:Number(json.format?.duration)};
