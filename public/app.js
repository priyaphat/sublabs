...
import { orderedUploadFiles, toggleUploadOrder } from '/shared/upload-order.mjs';
...
function uploadValidationError(files){
  if(!entryMode)return'กรุณาเลือกทำซับหรือทำเสียงก่อน';
  if(!files.length)return'';
  if(files.length>20)return'เลือกได้สูงสุด 20 คลิปต่อโปรเจกต์';
  if(files.reduce((sum,file)=>sum+file.size,0)>2*1024**3)return'ไฟล์รวมใหญ่เกิน 2GB';
  return'';
}
function clearPendingUpload(){
  pendingUploadFiles=[];pendingUploadOrder=[];$('#file').value='';$('#uploadFiles').replaceChildren();$('#uploadFiles').classList.add('hidden');$('#uploadOrderHint').classList.add('hidden');$('#uploadOrderActions').classList.add('hidden');
}
function renderUploadOrder(){
  const list=$('#uploadFiles'),positions=new Map(pendingUploadOrder.map((fileIndex,orderIndex)=>[fileIndex,orderIndex+1]));list.classList.remove('hidden');
  list.replaceChildren(...pendingUploadFiles.map((file,index)=>{const button=document.createElement('button'),number=document.createElement('span'),name=document.createElement('span'),position=positions.get(index);button.type='button';button.className=`upload-file-order${position?' ordered':''}`;button.setAttribute('aria-pressed',String(Boolean(position)));button.setAttribute('aria-label',position?`${file.name} ลำดับที่ ${position} คลิกเพื่อเอาออกจากลำดับ`:`เลือก ${file.name} เป็นคลิปถัดไป`);number.className='order-number';number.textContent=position||'–';name.className='file-name';name.textContent=file.name;button.append(number,name);button.addEventListener('click',()=>{pendingUploadOrder=toggleUploadOrder(pendingUploadOrder,index);renderUploadOrder()});return button}));
  $('#uploadOrderHint').classList.remove('hidden');$('#uploadOrderActions').classList.remove('hidden');const complete=pendingUploadOrder.length===pendingUploadFiles.length;$('#confirmUploadOrder').disabled=!complete;$('#confirmUploadOrder').textContent=complete?`เริ่มนำเข้า ${pendingUploadFiles.length} คลิป`:`เลือกลำดับ ${pendingUploadOrder.length}/${pendingUploadFiles.length}`;
}
async function upload(files){
  const error=uploadValidationError(files);if(error){$('#uploadError').textContent=error;return}if(!files.length)return;$('#uploadError').textContent='';
  const list=$('#uploadFiles');list.classList.remove('hidden');list.textContent=files.map((file,index)=>`${index+1}. ${file.name}`).join('  →  ');$('#uploadOrderHint').classList.add('hidden');$('#uploadOrderActions').classList.add('hidden');
  const form=new FormData();form.append('videoOrder',JSON.stringify(files.map(file=>file.name)));for(const file of files)form.append('videos',file);$('#pick').disabled=true;toast(files.length>1?`กำลังเตรียมและต่อ ${files.length} คลิปตามลำดับที่เลือก…`:'กำลังตรวจสอบวิดีโอ…');
  try{const created=await api('/api/projects',{method:'POST',body:form});clearPendingUpload();showProject(created)}catch(error){$('#uploadError').textContent=error.message;if(pendingUploadFiles.length>1)renderUploadOrder()}finally{$('#pick').disabled=false}
}
function prepareUpload(input){
  const files=Array.from(input||[]),error=uploadValidationError(files);if(error){$('#uploadError').textContent=error;return}if(!files.length)return;$('#uploadError').textContent='';pendingUploadFiles=files;pendingUploadOrder=files.length===1?[0]:[];
  if(files.length===1)upload(files);else renderUploadOrder();
}
...
$('#backHome').addEventListener('click',goHome);$('#chooseCaptionMode').addEventListener('click',()=>selectEntryMode('captions'));$('#chooseDubMode').addEventListener('click',()=>selectEntryMode('dub'));$('#pick').addEventListener('click',()=>{$('#file').value='';$('#file').click()});$('#file').addEventListener('change',event=>prepareUpload(event.target.files));$('#clearUploadOrder').addEventListener('click',()=>{pendingUploadOrder=[];renderUploadOrder()});$('#confirmUploadOrder').addEventListener('click',()=>{try{upload(orderedUploadFiles(pendingUploadFiles,pendingUploadOrder))}catch(error){$('#uploadError').textContent=error.message}});
for(const name of ['dragenter','dragover'])$('#drop').addEventListener(name,event=>{event.preventDefault();$('#drop').classList.add('drag')});
$('#drop').addEventListener('dragleave',()=>$('#drop').classList.remove('drag'));$('#drop').addEventListener('drop',event=>{event.preventDefault();$('#drop').classList.remove('drag');prepareUpload(event.dataTransfer.files)});
...
function applyStylePreset(style,node){const {enhanceFullHd,...visualStyle}=style||{};currentStyle={...currentStyle,...visualStyle};if(enhanceFullHd!==undefined){$('#enhanceFullHd').checked=Boolean(enhanceFullHd);$('#dubEnhanceFullHd').checked=Boolean(enhanceFullHd)}applyStyleToControls();clearStylePresetSelection();node?.classList.add('selected')}
...
function currentStylePreset(){return Object.fromEntries(stylePresetKeys.map(key=>[key,key==='enhanceFullHd'?$('#enhanceFullHd').checked:currentStyle[key]]).filter(([,value])=>value!==undefined))}
...
async function startRender(){if(!project||renderJobId)return;try{const value=await api(`/api/projects/${project.id}/exports`,json('POST',{style:currentStyle,enhanceFullHd:$('#enhanceFullHd').checked}));renderJobId=value.id;$('#render').disabled=true;$('#renderProgress').classList.remove('hidden');pollRender()}catch(error){toast(error.message)}}
...
async function startDubExport(){if(!project||dubExportId)return;try{const value=await api(`/api/projects/${project.id}/dub-exports`,json('POST',{enhanceFullHd:$('#dubEnhanceFullHd').checked}));dubExportId=value.id;$('#dubExportProgress').classList.remove('hidden');$('#exportDub').disabled=true;pollDubExport()}catch(error){toast(error.message)}}
